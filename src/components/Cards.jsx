                ...
                {products.map((product) => (
                    (selectedLoc === 'All Locations' || product.loc.city === selectedLoc) && (
                       ...
                       ${hoveredId === product.id ? "opacity-0" : "opacity-100"}`}
                            ...
                            {showMakeOffer && <button className="absolute bottom-4 left-1/2 -translate-x-1/2 w-[90%] bg-[#3F51B5] text-white font-semibold py-2 rounded-lg opacity-0 translate-y-2 transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0 group-hover:cursor-pointer">Make an offer</button>}
