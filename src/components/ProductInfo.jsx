...
const ProductInfo = ({product, ShowProductInfo, setShowProductInfo, ShowSendRequest, setShowSendRequest }) => {
  ...
  return (
    <div className={`popup-box ${ShowProductInfo ? '' : 'hidden'} !flex !flex-row p-8 gap-10 bg-white rounded-3xl max-w-4xl relative`}>
        ...
        <div className="w-1/2 flex flex-col gap-4 shrink-0">
            <img src={product.img} className="w-full h-[73%] object-cover rounded-2xl shadow-sm"/>
            <div className='flex flex-row gap-4 overflow-x-auto py-2 no-scrollbar'>
                {product.prodimg.map((url, index ) => (
                <div 
                key={index} 
        ...
        <div className='flex-1 flex flex-col '>
            ...
            <div className='flex flex-row mt-2 items-center'>
                <img src={product.avatar} className='w-16 h-auto rounded-full object-cover' />
                <p className='pl-5 text-xl font-Inter font-semibold '>{product.name}</p>

            </div>
            <div className='flex flex-row mt-4 items-center mb-2'>
                <MapPin size={28} className="text-black rounded-l-lg bg-[rgba(255,177,143,1)] p-1 " />
                <p className=' font-regular rounded-r-lg bg-[rgba(255,177,143,1)] text-xl px-2'>{product.location}</p>
            </div>
            <div className='mb-10'>
                <span>Tags:</span>
